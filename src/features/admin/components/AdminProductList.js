...
const AdminProductList = () => {
  ...
  // handlePage function to take next pageno as input and set the pageno state
  const handlePage = (pageno) => {
    setPage(pageno);
  };

  // useEffect to render on pageno,filter,dispatch,sort changes
  useEffect(() => {
    const pagination = { _page: pageno, limit: ITEMS_PER_PAGE };
    dispatch(fetchProductsByFiltersAsync({ filter, sort, pagination }));
  }, [dispatch, filter, sort, pageno]);

  // useEffect to set pageno = 1 on every sort
  useEffect(() => {
    setPage(1);
  }, [totalItems, sort]);
          ...
          <div className="flex items-baseline justify-between border-b border-gray-200 pb-6 pt-24">
            ...
            <div className="flex items-center">
              <Link
                ...
                className="py-1 px-2 mx-2 text-sm font-semibold text-white bg-green-600 items-center rounded-md hover:bg-green-500"
              > 
                Add New Item
              </Link>
          ...
          <div className="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:gap-x-8">
            {products.map((product) => (
              <div key={product.id} id={`product-detail-${product.id}`}>
                <ProductCard
                  product={product}
                  link={`/admin/product-detail/${product.id}`}
                />
              </div>
            ))}
          </div>
